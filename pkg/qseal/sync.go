...
const (
	SyncActionUnseal    SyncAction = "unseal"
	SyncActionSeal      SyncAction = "seal"
	SyncActionDoNothing SyncAction = "nothing"
)
...
func Sync(qsealRc qsealrc.Qsealrc) error {
	...
	for _, secret := range qsealRc.Secrets {
		...
		if err != nil {
			return fmt.Errorf("error getting sealed path for secret %s: %v", secret.Name, err)
		}
		action, err := decideSyncAction(sealedPath, secret)
		if err != nil {
			return fmt.Errorf("error deciding sync action for secret %s: %v", secret.Name, err)
		}
		...
		if ok && existingAction != SyncActionDoNothing {
			return fmt.Errorf(
